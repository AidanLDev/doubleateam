...
export default function BuildingCiCDForFree() {
  ...
  return (
    ...
    >
      <Text>
        CI/CD can speed up the process of updating websites. If you're going to
        be working on a website over the course of days, months and even years,
        you will spend a lot of time deploying changes you've made. This is
        where a CI/CD pipeline comes in. Push up changes to your GitHub and
        boom, those changes will be autoamtically deployed. No logging into and
        manually uploading your files to wherever they're being hosted from.
      </Text>
      <Text>
      ...
      <Text>
        ...
        <SmallBlogImg
          ...
          src={`${imgPath}create-s3-bucket.webp`}
        />
        Logically the region should be close to wherever the majority of users
        of your website are from to give a slightly better performance. You will
        also want to allow public access otherwise no one will be able to access
        your website:
        <SmallBlogImg
          alt="Allowing public access"
          src={`${imgPath}creating-bucket2.png`}
      ...
      <Text>
        ...
        and Access Management). We will want to give our CircleCI access to our
        S3 bucket so it can deploy our code to it later on down the line.
      </Text>
      <Text>
        ...
        it's access type, this gives this user access to perform actions using
        the AWS CLI but not console access.
        <SmallBlogImg alt="IAM user name" src={`${imgPath}iam-user-name.png`} />
        ...
        <SmallBlogImg
          ...
          src={`${imgPath}iam-permissions.png`}
        />
        Go through the rest of the create user Wizard and when you have added
        the user make sure to save the "Access Key ID" and "Secret access key"
      ...
      <Text>
        ...
        <SimpleLink link="https://github.com/">GitHub</SimpleLink>, create a
        repo and get it up there.
      </Text>
      <Heading>Set-up CircleCI account</Heading>
      <Text>
        Open up <SimpleLink link="https://circleci.com/">CircleCI</SimpleLink>{" "}
        and create an account, I reccomend signing up using your GitHub accoutn
        as you're going to link it to your CircleCI account anyways. Once you're
        in and have your GitHub account connected, you should be able to see
        your Repos. Click the "Set Up Project" button next to the repo you want
        to make a CI/CD pipeline for.
        <SmallBlogImg
          alt="CircleCI set up 1"
          src={`${imgPath}circleci-setup-yml-file.png`}
        />
        Choose Fast and then on the next screen select "Hello World", and just
        get through the rest of the initial start up by selecting "Use Existing
        Config" then "Start Building" as we now want to get into our Repo code
        and will start creating a config file to handle the CI pipeline.
      </Text>
      ...
      <Text>
        ...
        <code>config.yml</code>. In here you will want to add the following
        code:
      </Text>
      <Text>
      ...
      <Text>
        Push up those changes and watch the pipeline build on CircleCI! If all
        is well, then you should see the build go{" "}
        <span style={{ color: "green" }}>Green</span>.
      </Text>
      ...
      <Text>
        ...
        I had to update the <code>to: 's3://your-bucket'</code> to match my
        bucket name, before I was putting in the actual S3 bucket URL.
      </Text>
      <Text>
        <code>
          The user-provided path ./out/* does not exist. Exited with code exit
        ...
        being exluded as it was included in the <code>.gitignore</code> file.
        And the path for the folder just needed to be the folder name{" "}
        <code>out</code> without the "./folderName/*"
      </Text>
      <Text>
      ...
      <Text>
        I hope this was helpful and you managed to deploy a pipeline, if you
        have an issues please{" "}
        <SimpleLink link="/contact-us">get in touch</SimpleLink> and I will do
        what I can to help.
      </Text>
    </BlogLayout>
  );
